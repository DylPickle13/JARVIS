...
function compactInstructions(instructions: string, payload: any): string {
  ...
  text = text.replace(
    ...
    [
      ...
      "- Discover optional schemas with `load_tools`; its description is the canonical group inventory. Known valid direct calls auto-load registered lazy tools on the JARVIS runtime. Home controls, security/status, sensors, or door protocols (including explanations) => immediately `load_tools({groups:[\"operation_jarvis\"]})`, then its tools for actual device operations; files/shell/SSH/web remain permitted for diagnosing or maintaining JARVIS integration code. Never claim actions without tool results or bypass gates; cron checks use `jarvis_cron` unless OS cron/launchd is explicit.",
