...
- `46-local-pi-session-status.ts` — local Pi lifecycle heartbeat consumed by `jarvisd`; reports idle, running, waiting, and compacting, reconciling current `ctx.isIdle()` on heartbeats after compaction. Successful settled mobile-session turns can invoke the content-free APNs completion helper only behind its explicit private gate; no history replay or automatic Pi reload. jarvisd derives offline and fail-closed unknown states from fixed tmux evidence.
