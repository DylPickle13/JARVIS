# Native app implementation history

> **Archive, not an active backlog or deployment record.** These are the former
> documentation index and consolidated implementation plans. Build-specific
> status, session counts, notification privacy contracts, and candidate labels
> may be superseded. Their original wording is retained for traceability.
> Do not execute historical rollout instructions without a fresh owner-approved
> plan and verification of the actual runtime and signed artifacts.

See the [documentation index](README.md), [architecture](architecture.md),
