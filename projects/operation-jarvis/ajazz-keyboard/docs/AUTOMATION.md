# Basement presence lighting watcher

## Active components

- Local user LaunchAgent **`com.jarvis.ajazz-keyboard-watch`** runs `watch.py --watch`.
- Private JARVIS scheduler job **`Keyboard lights`**, ID **`job_41fb6dd73fce`**, runs `watch.py --alerts` every minute using `__direct_stdout__`. It only relays alerts and checks watcher health; it never sends keyboard commands.
...
- **Either device nearby:** first fresh basement `nearby` report resumes randomly selected liked effects, purple `#9933FF`, highest brightness, fastest speed. Ordinary effect changes remain at least 60 seconds apart, excluding presence transitions. Do not repeat the last successful coloured rotation effect.
- **Both away:** first fresh basement `away` report applies **`ripples`, `#9933FF`, `highest`, `fastest`, `left_to_right`** once. Repeated away checks and watcher restarts do not resend a successful away profile. Black RGB is no longer the requested away behavior.
...
The existing `ajazz.py set` transport is unchanged: one validated 65-byte output report, exact model/interface checks, shared/non-exclusive HID access, no new protocol, input read, firmware operation, USB cutoff or recovery write.
...
- `state.json` version 3: timestamps, counters, last coloured effect, `away_applied`, pending marker and mode. Versions 1/2 are validated and migrated without clearing pending. Version-2 black RGB is not treated as already-applied purple ripples.
...
`cycle.py --once` retains the legacy bounded/two-check behavior for explicit manual use; it is no longer scheduled. Do not manually run it alongside the watcher. `cycle.py --acknowledge-uncertain` remains an explicit owner-only recovery decision: it sends nothing, clears the pending block and resets the away latch; a later fresh watcher check may then send a command. Never invoke acknowledgment automatically.

## Validation

72 offline tests pass, including the unchanged CLI tests, state migration, fast transitions with minute-spaced rotation, unknown/stale no-write behavior, deadline rechecks, known-failure cooldown despite presence flips, uncertain outcome blocking, singleton locks, restart suppression, and durable error/recovery relay. Tests use mocks and send no keyboard reports.
