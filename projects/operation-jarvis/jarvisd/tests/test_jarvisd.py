...
jarvisd = load_daemon("jarvisd_under_test", MODULE_PATH)
...
class DaemonUnitTests(unittest.TestCase):
    ...
    def test_mobile_pi_session_states_use_fresh_lifecycle(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                {"sessionID": 10, "lifecycle": "offline", "active": False},
            ],
        )
    ...
    def test_mobile_pi_session_states_fail_closed_when_probe_is_unavailable(self):
        unknown = [
            ...
            {"sessionID": 10, "lifecycle": "unknown", "active": None},
        ]
    ...
    def test_event_bridge_targets_only_jarvisd_and_uses_neutral_controls(self):
        adapter_path = Path(__file__).resolve().parents[2] / "jarvis.py"
...
class HTTPTests(unittest.TestCase):
    ...
    def test_named_room_routes_forward_only_the_selected_speaker(self):
        def reply(handler, turn_id=None, speaker_id='pi'):
            handler._send(200, {'ok': True, 'speakerID': speaker_id})
        with mock.patch.object(jarvisd.Handler, '_room_audio', autospec=True, side_effect=reply) as relay:
            for speaker in ('pi', 'mac'):
                status, _, body = self.request('GET', '/api/v1/room-audio/' + speaker, token='api-secret')
                self.assertEqual(status, 200)
                self.assertEqual(json.loads(body)['speakerID'], speaker)
                self.assertEqual(relay.call_args.kwargs['speaker_id'], speaker)
                status, _, body = self.request('POST', '/api/v1/room-audio/' + speaker + '/stop', {'turnID': 'a'*32}, token='api-secret')
                self.assertEqual(status, 200)
                self.assertEqual(relay.call_args.args[1], 'a'*32)
                self.assertEqual(relay.call_args.kwargs['speaker_id'], speaker)
    ...
    def test_room_audio_requires_api_auth_and_an_exact_turn(self):
        with mock.patch.object(jarvisd.Handler, "_room_audio") as relay:
            ...
            self.assertEqual(self.request("GET", "/api/v1/room-audio/mac")[0], 401)
            self.assertEqual(self.request("POST", "/api/v1/room-audio/mac/stop", {"turnID": "a"*32})[0], 401)
